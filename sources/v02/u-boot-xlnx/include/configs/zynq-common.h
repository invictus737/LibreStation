...
#define CONFIG_EXTRA_ENV_SETTINGS	\
	...
	"ramboot_verbose=adi_hwref;echo Copying Linux from DFU to RAM... && " \
		...
		"fi; " \
			"envversion;setenv bootargs console=ttyPS0,115200 maxcpus=2 rootfstype=ramfs root=/dev/ram0 rw earlyprintk clk_ignore_unused cpuidle.off=1 idle=poll mitigations=off && " \
	...
	"qspiboot=set stdout nulldev;adi_hwref;test -n $PlutoRevA || gpio input 14 && set stdout serial@e0001000 && sf probe && sf protect lock 0 100000 && run dfu_sf;  " \
		...
		"run read_sf && run adi_loadvals; " \
			"envversion;setenv bootargs console=ttyPS0,115200 maxcpus=2 rootfstype=ramfs root=/dev/ram0 rw quiet loglevel=4 clk_ignore_unused cpuidle.off=1 idle=poll mitigations=off && " \
