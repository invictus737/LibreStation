...
module util_upack2_timestamp #(
  ...
  // Limit of how far a timestamp can be in the future as a multiple of timestamp_every.
  // Timed TX users commonly schedule several milliseconds ahead; keep this large
  // enough for high sample-rate operation while still rejecting clearly bad timestamps.
  parameter TIMESTAMP_LIMIT_EVERY_MULTIPLE = 4096
) (
    // DMA clock
    input dma_clk,
    ...
    // Reset from DAC/TDD logic. The board integration can source this from
    // another clock domain, so synchronize it before use in each domain.
    input reset,
    ...
    */
    ...
    (* QUANTULUM_LTD_FALSE_PATH = 1 *) (* ASYNC_REG = "TRUE" *) reg reset_dma_meta = 1'b1;
    (* ASYNC_REG = "TRUE" *) reg reset_dma = 1'b1;
    (* QUANTULUM_LTD_FALSE_PATH = 1 *) (* ASYNC_REG = "TRUE" *) reg reset_dac_meta = 1'b1;
    (* ASYNC_REG = "TRUE" *) reg reset_dac = 1'b1;

    always @(posedge dma_clk) begin
        reset_dma_meta <= reset;
        reset_dma <= reset_dma_meta;
    end

    always @(posedge dac_clk) begin
        reset_dac_meta <= reset;
        reset_dac <= reset_dac_meta;
    end
    ...
    always @(posedge dma_clk) begin
        if (reset_dma || !timestamp_en || !s_axis_xfer_req) begin
    ...
    always @(posedge dma_clk) begin
        if (reset_dma || !s_axis_xfer_req) begin
            // Reset discard reg
            timestamp_check_discard <= 'b0;
            timestamp_discard_count <= 0;
    ...
    always @(posedge dac_clk) begin
        if (reset_dac) begin
            transfer_start_rising_delayed_dac <= 'b0;
        end else begin
    ...
    assign reset_upack = reset_dac || transfer_start_rising_stretched_dac;
